...
Use `connectUrl` to point every hosted window at a matching Connect deployment in staging or local environments (production is the default); the deployment must serve the hosted `/launch` page, which this SDK version opens for every flow. It must be `https:`, or `http:` only for a loopback host (`localhost`, `127.0.0.1`, `[::1]`) — a plaintext hosted page on any other host is rejected, since its origin would become the one the SDK trusts for the auth callback. Use `sessionStorage: false` for tests or controlled apps that do not want SDK-managed browser storage. Custom `popup`, `sessionStorage`, `fetch`, and `createState` implementations are supported for tests and custom integrations. A login also claims its result from Rare API whenever the page becomes visible again or the hosted window is found closed (iOS Safari suspends the page while the hosted window is in front, and the callback the window posts meanwhile never arrives); `popup.visibilityEvents` overrides the page-visibility source for tests and non-browser hosts.
